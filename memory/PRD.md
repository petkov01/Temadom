...
- **Separated polling from result**: Polling returns ~150 bytes (no base64). Full result fetched from `/api/ai-designer/result/{design_id}` — fixes mobile 95% stuck issue
- **Parallel Processing**: Photos + product scraping via `asyncio.gather`
- **Navbar Logo**: 80x80px on all screens (h-20)
- **Testing**: iterations 78-80, all passed
...
### P3 (Future)
- Job ads module, Facebook login, Advanced analytics
