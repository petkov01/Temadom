    ...
    try:
        ...
        async def process_single_photo(idx, photo_b64):
            ...
            try:
                ...
                render_prompt = f"""АНАЛИЗИРАЙ САМО ЕДНАТА снимка на {room_type_name}.
НЕ променяй архитектурата, размерите, позициите!

✅ КОПИРАЙ ТОЧНО от снимката:
- Същите стени, ъгли, прозорци, врати — 100% НЕПРОМЕНЕНИ
- Същият ъгъл/перспектива на камерата — PIXEL-PERFECT
- Същото осветление, цветова температура
- Размери: {width}м x {length}м x {height}м
- Фиксирани ВиК: тоалетна, мивка, вана/душ, бойлер — НА СЪЩИТЕ МЕСТА
{style_context}
✅ ГЕНЕРИРАЙ САМО:
- {style_desc.upper()} стил — МЕБЕЛИ КЪДЕТО ИМА ПРАЗНИНА (по размерите)
- Смесители/аксесоари В СЪОТВЕТНИТЕ позиции (не нови!)
- Нови настилки и облицовки — плочки/паркет/мазилка
- LED осветление, минималистични акценти (дърво/мрамор)
{f'- Клиентски бележки: {notes}' if notes else ''}

НЕ ИЗМИСЛЯЙ нови стени/прозорци/врати!
Копирай 100% архитектурата от снимката!
Резултат: фотореалистичен рендер, ТОЧНО същия ъгъл, професионална интериорна фотография."""
