  ...
  return (
    <nav className="glass-nav sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-[88px] lg:h-24">
          {/* Left: Logo */}
          <div className="flex items-center">
            <Link to="/" className="flex items-center gap-2.5" data-testid="logo-link">
              <img src="/logo-temadom.png" alt="TemaDom" className="h-20 lg:h-24 w-auto" style={{ filter: 'drop-shadow(0 0 12px rgba(246, 195, 106, 0.7))' }} />
            </Link>
          </div>
      ...
      <section className="relative overflow-hidden noise-overlay" style={{ minHeight: '88vh' }}>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 py-16 md:py-24 flex flex-col items-center">
          ...
          <div className="mb-8 animate-slideUp">
            <img src="/logo-temadom.png" alt="TemaDom" className="h-40 md:h-52 w-auto mx-auto animate-float" style={{ filter: 'drop-shadow(0 0 20px rgba(246, 195, 106, 0.8))' }} />
          </div>
...
const GoogleAuthCallback = () => {
  ...
  useEffect(() => {
    ...
    // REMINDER: DO NOT HARDCODE THE URL, OR ADD ANY FALLBACKS OR REDIRECT URLS, THIS BREAKS THE AUTH
    // Extract session_id from URL fragment (#session_id=...)
    ...
    // Exchange session_id for JWT via our backend
    axios.post(`${API}/auth/google/callback`, { session_id: sessionId })
      .then(res => {
        if (res.data.token) {
          loginWithToken(res.data.token, res.data.user);
          toast.success(res.data.is_new ? 'Регистрацията с Google е успешна!' : 'Добре дошли отново!');
          navigate('/');
        }
      })
      ...
      <Card className="w-full max-w-md" data-testid="register-form">
        ...
        <CardContent>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* Google Auth option for clients */}
            {userType === 'client' && (
              <>
                ...
                <Button type="button" variant="outline" className="w-full" data-testid="google-register-btn"
                  onClick={() => {
                    // REMINDER: DO NOT HARDCODE THE URL, OR ADD ANY FALLBACKS OR REDIRECT URLS, THIS BREAKS THE AUTH
                    const redirectUrl = window.location.origin + '/auth/callback';
                    window.location.href = `https://auth.emergentagent.com/?redirect=${encodeURIComponent(redirectUrl)}`;
